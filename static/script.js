...
// Função para salvar CEP na sessão
function salvarCepNoCarrinho(cep) {
    fetch('/api/salvar-frete', {
        method: 'POST',
        headers: {
...
// Modifique a função calcularFrete no script.js para salvar o CEP:
async function calcularFrete(cep) {
    // ... código existente ...
    
    // Salvar CEP na sessão
    salvarCepNoCarrinho(cep);
    
    // ... resto do código ...
}
function mostrarMsg(texto, tipo) {
    const el = document.getElementById("status-popup");

    el.textContent = texto;
    el.className = "status-popup " + tipo;
